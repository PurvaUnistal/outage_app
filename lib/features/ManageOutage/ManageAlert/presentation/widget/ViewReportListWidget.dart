...
class ViewReportListWidget extends StatelessWidget {
  ...
  const ViewReportListWidget({super.key, required this.listOfViewIncident, required this.checkIncidentType});

  @override
  Widget build(BuildContext context) {
    Set<Marker> markers = Set();
    return checkIncidentType == false ? ListView.builder(
          ...
          return Card(
            child: ListTile(
              onTap :() async {
                ...
                await SharedPref.setString(key: PrefsValue.incidentTypeId,value: data.incidentTypeId!);
                await SharedPref.setString(key: PrefsValue.incidentId,value: data.incid!);
                Navigator.push(context,MaterialPageRoute(builder: (context) =>ReportDetailsView(
              ...
              title:_rowWidget(
                  title: "Report Status : ",
                  subtitle:data.actionStatus == "0" ? "New" : data.actionStatus == "1" ? "In Progress" : "Completed",
                  color: data.actionStatus == "0" ? Colors.red : data.actionStatus == "1" ? Colors.yellow.shade800 : Colors.green.shade800,
              ),
              subtitle: Column(
                children: [
                  Divider(),
                  _rowSubtitleWidget(
                      title: "Incident Type : ",
                      subtitle:listOfViewIncident[i].incidenttype.toString()
                  ),
                  _rowSubtitleWidget(
                      title: "Priority : ",
                      subtitle:listOfViewIncident[i].priority.toString()
                  ),
                  _rowSubtitleWidget(
                      title: "Report Date : ",
                      subtitle: listOfViewIncident[i].createdAt.toString()
                  ),
                  Divider(),
                  Container(
  ...
  Widget _rowWidget({required String title, required String subtitle, required Color color}){
    return Row(
