...
import 'package:outage_app/features/Manage/IncidentManage/domain/bloc/incident_manage_event.dart';
import 'package:outage_app/features/Manage/IncidentManage/domain/bloc/incident_manage_state.dart';
import '../widget/action_items_widget.dart';
import '../widget/tab_item.dart';

class IncidentManageView extends StatefulWidget {
  const IncidentManageView({super.key});

  @override
  State<IncidentManageView> createState() => _IncidentManageViewState();
}

class _IncidentManageViewState extends State<IncidentManageView>
  ...
  void initState() {
    ...
    BlocProvider.of<IncidentManageBloc>(
      context,
    ).add(IncidentManageLoadEvent(context: context));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarWidget(title: AppString.incidentManage, boolLeading: true),
      body: SafeArea(
        child: BackgroundInfoWidget(
          child: BlocBuilder<IncidentManageBloc, IncidentManageState>(
            builder: (context, state) {
              if (state is FetchIncidentManageDataState) {
  ...
  Widget _itemBuilder({required FetchIncidentManageDataState dataState}) {
    return Padding(
  ...
  Widget _searchPriorityController({
    required FetchIncidentManageDataState dataState,
  }) {
  ...
  Widget _tabControllerWidget({required FetchIncidentManageDataState dataState}) {
    return Flexible(
      child: Column(
        children: [
          ClipRRect(
            ...
            child: Container(
              ...
              child: TabBar(
                ...
                onTap: (index) {
                  BlocProvider.of<IncidentManageBloc>(context).add(
  ...
  Widget _itemWidget({required FetchIncidentManageDataState dataState}) {
  ...
  Future<void> _handleRefresh() async {
    ...
    BlocProvider.of<IncidentManageBloc>(!context.mounted ? context : context).add(
